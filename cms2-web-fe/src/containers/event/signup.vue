  ...
  <div class="container signup bg">
    ...
    <div class="form-horizontal">
      ...
      <div class="form-group" v-if="eventStore.eventEnroll.event_type==1 || eventStore.eventEnroll.event_type==2">
        ...
        <div class="col-sm-8">
          <div class="radio">
            <label v-for="(value,key,index) in regType2">
      ...
      <div class="form-group"  v-if="1===0">
        ...
        <div class="col-sm-4">
          <div class="radio">
            <label v-for="(value,key,index) in secondEventType2">
...
<!--
      ...
      <div class="form-group">
        ...
        <div class="col-sm-8">
          <div class="radio"  v-if="sex==0">
            <!-- {{JSON.stringify(manSize2)}} -->
            <label v-for="(value,key,index) in manSize2">
              <input v-model="mdress_size" v-bind:value="key" type="radio">
              {{value}}
            </label>
            <!-- {{JSON.stringify(manSize)}} -->
          ...
          <div class="radio" v-if="sex==1">
            <label v-for="(value,key,index) in womenSize2">
      ...
      <div class="form-group">
        ...
        <div class="col-sm-8">
          <div class="radio"  v-if="sex==0">
            <label v-for="(value,key,index) in manShoes_size2">
          ...
          <div class="radio" v-if="sex==1">
            <label v-for="(value,key,index) in womenShoes_size2">
      ...
      <div class="form-group">
        <div class="col-sm-offset-3 col-sm-9">
          <button type="button" v-on:click="enrollSubmit" class="btn btn-default">报名</button>
...
export default {
  ...
  watch: {
    ...
    'eventStore.eventEnroll' (newVal, oldVal) {
      // console.log(newVal)
      let that = this
      if ('man' in newVal && 'shoes_size' in newVal.man && Object.keys(newVal.man.shoes_size).length > 0) {
        // that.mshoes_size = Object.keys(that.eventStore.eventEnroll.man.shoes_size)[0]
        that.manShoes_size2 = newVal.man.shoes_size
      }
      if ('man' in newVal && 'size' in newVal.man && Object.keys(newVal.man.size).length > 0) {
        // that.mdress_size = Object.keys(that.eventStore.eventEnroll.man.size)[0]
        that.manSize2 = newVal.man.size
        for (let i in newVal.man.size) {
          if (newVal.man.size[i] === this.userStore.user.default_dress) {
            that.mdress_size = i
          }
        }
      }

      if ('women' in newVal && 'size' in newVal.women && Object.keys(newVal.women.size).length > 0) {
        // that.wdress_size = Object.keys(that.eventStore.eventEnroll.women.size)[0]
        that.womenSize2 = newVal.women.size
        for (let i in newVal.women.size) {
          if (newVal.women.size[i] === that.userStore.user.default_dress) {
            that.wdress_size = i
          }
        }
      }
      if ('women' in newVal && 'shoes_size' in newVal.women && Object.keys(newVal.women.shoes_size).length > 0) {
        // that.wshoes_size = Object.keys(that.eventStore.eventEnroll.women.shoes_size)[0]
        that.womenShoes_size2 = newVal.women.shoes_size
      }

      if ('second_event_type' in newVal && Object.keys(newVal.second_event_type).length > 0) {
        that.second_event_type = Object.keys(newVal.second_event_type)[0]
        that.secondEventType2 = newVal.second_event_type
      }

      if ('reg_type' in newVal && Object.keys(newVal.reg_type).length > 0) {
        that.reg_type = Object.keys(newVal.reg_type)[0]
        that.regType2 = newVal.reg_type
      }
    }
  }
}
