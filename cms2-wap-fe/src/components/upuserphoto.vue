<template>
  <span>
  <form method="post" ref="upimageForm" v-bind:action="config.img.avatar" style="display:inline" v-bind:target="ifameId" enctype="multipart/form-data">
        <a href="javascript:void(0)" class="alink" style="line-height:35px;" v-on:click="addPic">{{upimgtitle}} </a>
        <input type="file" name="avatar" ref="upimageFile" @change="onFileChange" style="display: none;">
  ...
  methods: {
    ...
    onFileChange (e) {
      ...
      reader.onloadend = function () {
        _this.$emit('getImageData', _this.upimgtype, {
          data: {
            avatar: this.result
          }
        })
